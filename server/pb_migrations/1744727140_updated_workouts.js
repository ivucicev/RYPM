/// <reference path="../pb_data/types.d.ts" />
migrate((db) => {
  const dao = new Dao(db)
  const collection = dao.findCollectionByNameOrId("u01gz4m571bnw0d")

  collection.listRule = "@request.auth.id != \"\"  && @request.auth.userId = user.id"
  collection.viewRule = "@request.auth.id != \"\"  && @request.auth.userId = user.id"
  collection.createRule = "@request.auth.id != \"\"  && @request.auth.userId = user.id"
  collection.updateRule = "@request.auth.id != \"\"  && @request.auth.userId = user.id"
  collection.deleteRule = "@request.auth.id != \"\"  && @request.auth.userId = user.id"

  return dao.saveCollection(collection)
}, (db) => {
  const dao = new Dao(db)
  const collection = dao.findCollectionByNameOrId("u01gz4m571bnw0d")

  collection.listRule = "@request.auth.id != \"\"  && @request.auth.id = user.id"
  collection.viewRule = "@request.auth.id != \"\"  && @request.auth.id = user.id"
  collection.createRule = "@request.auth.id != \"\"  && @request.auth.id = user.id"
  collection.updateRule = "@request.auth.id != \"\"  && @request.auth.id = user.id"
  collection.deleteRule = "@request.auth.id != \"\"  && @request.auth.id = user.id"

  return dao.saveCollection(collection)
})
